...
The code writes tables and plots first, then generates reports from those artifacts. This is safer than writing narrative first and then trying to remember to update it when results change. It also allows the report generation code to be run independently of the experiment execution, which is useful for fast iteration on the write-up without retraining.